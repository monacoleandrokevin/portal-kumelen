...
export async function isAllowedByWorkspace(email) {
  ...
  // 🔴 CLAVE: forzamos autorización para que genere y adjunte el access_token
  await auth.authorize(); // <- sin esto, el fetch puede ir sin Authorization
  ...
  return allowedOUs.some((base) => orgUnitPath.startsWith(base));
}
